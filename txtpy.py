...
# 设置目录路径和小数位数
dir_path = 'C:\\yolov7\\yolov7dataset\\train\\labels'
decimal_places = 6
...
for filename in os.listdir(dir_path):
    if filename.endswith('.txt'):
        ...
        # 使用正则表达式匹配小数，并进行格式化
        formatted_data = re.sub(r'(\d+\.\d*)', lambda x: '{:.{}f}'.format(float(x.group(0)), decimal_places), filedata)
...
# 设置目录路径和小数位数
dir_path = 'C:\\Users\\az567\\Desktop\\yolov7datasetvvv\\valid\\labels'
...
for filename in os.listdir(dir_path):
    if filename.endswith('.txt'):
        ...
        # 使用正则表达式匹配小数，并进行格式化
        formatted_data = re.sub("2","1", filedata)
